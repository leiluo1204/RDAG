...
class DAGERC_fushion(nn.Module):
    ...
    def __init__(self, args, num_class, reason_steps=None):
        super().__init__()
        self.args = args
        # gcn layer
        self.steps = reason_steps if reason_steps is not None else [0, 0]
        self.reason_modules = nn.ModuleList([
            ReasonModule(in_channels=args.hidden_dim, processing_steps=self.steps[0], num_layers=1),
            ReasonModule(in_channels=args.hidden_dim, processing_steps=self.steps[1], num_layers=1)
        ])
    ...
    def forward(self, features, adj, s_mask, s_mask_onehot, lengths):
        ...
        for l in range(self.args.gnn_layers):
            ...
            for i in range(1, num_utter):
                ...
                # print('H1', H1.size())
                # print('----------------------------------------------------')
            H1 = self.reason_modules[0](H, self.args.batch_size, H1)
            if (len(H) > 0):
                H2 = self.reason_modules[1](H[-1], self.args.batch_size, H1)
            fea = H1 + H2
            H.append(fea)
        H.append(features)
